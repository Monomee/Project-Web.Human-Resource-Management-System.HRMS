...
Sơ đồ bối cảnh thể hiện ranh giới hệ thống tròn ở trung tâm (`HRMS Core System`) tương tác với các thực thể hình chữ nhật bao quanh theo đúng chuẩn thiết kế bối cảnh hệ thống của Hassan Gomaa.
...
graph TD
    ...
    HR[Quản trị HR]
    Excel[Tệp chấm công Excel / Thiết bị]
    Notif[Hệ thống Thông báo]

    HRMS((HRMS Core System))

    %% Top: Nhân viên
    NV -->|"Đăng nhập, Check-in/out, Gửi đơn"| HRMS
    HRMS -->|"Bảng công, Phiếu lương, Trạng thái đơn"| NV

    %% Right: Quản lý trực tiếp
    QL -->|"Phê duyệt / Từ chối đơn"| HRMS
    HRMS -->|"Danh sách đơn chờ duyệt"| QL

    %% Bottom: Quản trị HR
    HR -->|"Hồ sơ nhân viên, Hợp đồng, Khóa kỳ công, Tính lương"| HRMS
    HRMS -->|"Báo cáo nhân sự & Bảng lương công ty"| HR

    %% Left: Tệp dữ liệu bên ngoài
    Excel -->|"Tải lên dữ liệu chấm công"| HRMS

    %% External Notification
    HRMS -->|"Gửi email & thông báo trạng thái"| Notif
```
...
sequenceDiagram
    ...
    participant View as LoginView <<boundary>>
    participant Ctrl as AuthControl <<control>>
...
sequenceDiagram
    ...
    participant View as AttendanceView <<boundary>>
    participant Ctrl as AttendanceControl <<control>>
...
sequenceDiagram
    ...
    participant SubView as RequestSubmissionView <<boundary>>
    participant Ctrl as RequestWorkflowControl <<control>>
    ...
    participant AppView as ApprovalView <<boundary>>
    participant DB as Database
...
sequenceDiagram
    ...
    participant View as ContractView <<boundary>>
    participant Ctrl as ContractControl <<control>>
...
sequenceDiagram
    autonumber
    actor HR as Quản trị HR
    participant View as PayrollView <<boundary>>
    participant Ctrl as PayrollControl <<control>>
    participant PeriodEntity as TimesheetPeriod <<entity>>
    participant DB as Database
